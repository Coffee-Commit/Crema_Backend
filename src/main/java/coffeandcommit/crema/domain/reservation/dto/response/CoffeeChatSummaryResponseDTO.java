...
public class CoffeeChatSummaryResponseDTO {
    ...
    private Integer pendingCount;    // 대기 중 커피챗 수 (status = PENDING), 취소 또는 거절된 커피챗은 포함하지 않음
    private Integer confirmedCount;  // 예정된 커피챗 수 (status = CONFIRMED)
    private Integer completedCount;  // 완료된 커피챗 수 (status = COMPLETED)
